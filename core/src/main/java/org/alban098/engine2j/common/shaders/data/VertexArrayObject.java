...
import org.alban098.engine2j.common.shaders.data.vbo.FloatVertexBufferObject;
import org.alban098.engine2j.common.shaders.data.vbo.IntegerVertexBufferObject;
...
public final class VertexArrayObject implements Cleanable {
  ...
  public boolean batch(Renderable renderable) {
    ...
    RenderElement renderableComponent = renderable.getRenderable();
    Transform transformComponent = renderable.getTransform();
    if (renderableComponent != null) {
      // if transform is needed, buffer it to the SSBO
      if (ssbo != null) {
        if (transformComponent != null) {
          ssbo.buffer(transformComponent.toFloatBuffer());
        } else {
